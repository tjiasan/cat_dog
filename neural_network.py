...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    do_nothing[0] = 1
    animal, s_t = game_state.draw_frame()


    # saving and loading networks
    saver = tf.train.Saver()
    sess.run(tf.initialize_all_variables())
    checkpoint = tf.train.get_checkpoint_state("/home/jason/catdog/save")
...
##                    y_batch.append(r_batch[i] + GAMMA* np.max(readout_j1_batch[i]))
        ...
        if t % 10000 == 0:
            ...
            saver.save(sess, '/home/jason/catdog/save/' + GAME + '-dqn', global_step = t)
        ...
        if t <= OBSERVE:
            ...
            state = "train"
        if t %100 ==0:
            print "TIMESTEP", t, "/ STATE", state, "/ LINES", "/ ACTION",action_index ,"/ REWARD", r_t, "/ Q_MAX %e" % np.max(readout_t)
